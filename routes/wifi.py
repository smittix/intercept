        ...
        try:
            ...
            for line in result.stdout.split("\n"):
                ...
                if line.startswith("Interface"):
                    ...
                    interfaces.append(
                        {"name": current_iface, "type": iface_type, "monitor_capable": True, "status": "up"}
                    )
                    current_iface = None
            ...
            try:
                ...
                for line in result.stdout.split("\n"):
                    if "IEEE 802.11" in line:
                        iface = line.split()[0]
                        interfaces.append({"name": iface, "type": "managed", "monitor_capable": True, "status": "up"})
            except Exception:
                pass
        except Exception as e:
    ...
    if action == "start":
        if check_tool("airmon-ng"):
            try:
                ...
                result = subprocess.run(["airmon-ng", "start", interface], capture_output=True, text=True, timeout=15)
        ...
        if check_tool("airmon-ng"):
            try:
                subprocess.run(
                    ["airmon-ng", "stop", app_module.wifi_monitor_interface or interface],
                    capture_output=True,
                    text=True,
                    timeout=15,
                )
...
def start_wifi_scan():
    ...
    with app_module.wifi_lock:
        ...
        cmd = ["airodump-ng", "-w", csv_path, "--output-format", "csv,pcap", "--band", band, interface]
...
def send_deauth():
    ...
    try:
        cmd = ["aireplay-ng", "--deauth", str(count), "-a", target_bssid, "-c", target_client, interface]
...
def check_handshake_status():
    ...
    try:
        if target_bssid and is_valid_mac(target_bssid):
            result = subprocess.run(
                ["aircrack-ng", "-a", "2", "-b", target_bssid, capture_file], capture_output=True, text=True, timeout=10
            )
            output = result.stdout + result.stderr
...
def check_pmkid_status():
    ...
    try:
        ...
        result = subprocess.run(
            ["hcxpcapngtool", "-o", hash_file, capture_file], capture_output=True, text=True, timeout=10
        )
